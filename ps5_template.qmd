...
us_states = gpd.read_file('/Users/apple/Desktop/class/Dap-2/problem_sets/ps5/Ruyu-problem-set-5-paired-part/cb_2018_us_state_5m')
# aggregate data 'State' & 'Enforcement Count'
...
ax.set_title('Enforcement Actions by State', fontdict={'fontsize': '18', 'fontweight': '3'})
...
```{python}
zip_shapefile_path = '/Users/apple/Desktop/class/Dap-2/problem_sets/ps5/Ruyu-problem-set-5-paired-part/tl_2020_us_zcta520'
zip_gdf = gpd.read_file(zip_shapefile_path)

# Load population data CSV
population_csv_path = '/Users/apple/Desktop/class/Dap-2/problem_sets/ps5/Ruyu-problem-set-5-paired-part/DECENNIALDHC2020.P1_2024-11-09T125756'
population_df = pd.read_csv(population_csv_path)

# Ensure ZIP Code column in population data matches the ZIP Code field in shapefile
population_df['ZCTA'] = population_df['ZCTA'].astype(str)

# Merge ZIP Code shapefile with population data
zip_gdf = zip_gdf.merge(population_df, left_on="ZCTA5CE20", right_on="ZCTA", how="left")
print(zip_gdf.head())
```
...
district_shapefile_path = '/path/to/US_Attorney_Districts_Shapefile.shp'  # Update this path to your district shapefile location
...
# Perform spatial join between ZIP Code and District shapefiles
zip_district_join = gpd.sjoin(zip_gdf, district_gdf, how="inner", op="intersects")

# Aggregate population to get total population in each district
district_population = zip_district_join.groupby('district_field_name')['P1_TOTAL_POPULATION'].sum().reset_index()
district_population.columns = ['District', 'Total Population']  # Rename columns for clarity
...
```{python}
# Merge enforcement actions with district population data
district_data = district_population.merge(enforcement_actions_df, on='District', how='left')
district_data['Action Count'] = district_data['Action Count'].fillna(0)  # Fill missing action counts with 0

# Calculate the ratio of enforcement actions per capita
district_data['Actions Per Capita'] = district_data['Action Count'] / district_data['Total Population']

# Merge with district GeoDataFrame for mapping
district_map_gdf = district_gdf.merge(district_data, left_on='district_field_name', right_on='District')

# Plot the map
import matplotlib.pyplot as plt

fig, ax = plt.subplots(1, 1, figsize=(12, 8))
district_map_gdf.plot(column='Actions Per Capita', cmap='Reds', linewidth=0.8, ax=ax, edgecolor='0.8', legend=True)
